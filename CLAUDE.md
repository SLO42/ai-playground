...
| **Failure log** | `docs/fails.md` on **`v2-main`** — the AUTHORITATIVE copy, 31 entries, head **F-061**. Scan it before starting; append when a gate fails first-try. **⚠️ An `F-NNN` id is AMBIGUOUS across copies (measured 2026-08-06).** The `v2` copy is a FORK with its own numbering — 47 entries, head F-057, holding 28 entries absent upstream and missing 12 that exist upstream. **F-016, F-020, F-045, F-046 name DIFFERENT failures in each copy**, and every F-id cited in this file means the **authoritative** copy — which resolves WRONG in a `v2` worktree, where 25 of the 26 worktrees on disk live. **Allocate a new id from the highest head across BOTH copies (today F-062+), never from a branch-local head** — branch-local allocation is exactly what caused the collisions, and a one-sided renumber relocates a collision rather than resolving it. Measurement + reconciliation options: `docs/FAILS-DIVERGENCE.md` (operator-gated). |
