...
| **SurrealDB migrations** | `const mNNNN_name: Migration = {...}` **inside `src/lib/server/db/schema.ts`** → exported `schemaMigrations`. **There is NO migrations directory and NO per-migration files.** Head is `m0080_maintenance_loops`. |
