...
| **SurrealDB migrations** | `const mNNNN_name: Migration = {...}` **inside `src/lib/server/db/schema.ts`** → exported `schemaMigrations`. **There is NO migrations directory and NO per-migration files.** Head is `m0085` (`m0085` thinking-ledger vocabulary — `agent_event` 'consult' + `scene_event` 'pm_review', analytics-visibility AV-3, 2026-07-25; prior `m0083`/`m0084` same wave, `m0082_autonomy_status`). |
