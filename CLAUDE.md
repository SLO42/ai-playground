...
| **SurrealDB migrations** | `const mNNNN_name: Migration = {...}` **inside `src/lib/server/db/schema.ts`** → exported `schemaMigrations`. **There is NO migrations directory and NO per-migration files.** Head is `m0086` (`m0086_boot_skip_ledger` — extends the SD-2 autonomy_status singleton with a per-subsystem boot ledger, analytics-visibility AV-4, 2026-07-26; prior `m0083`/`m0084`/`m0085` same wave). **Allocated but not yet built: `m0087` MODEL-LADDER-SPEC, `m0088` TASK-BOARD-SPEC — allocate from the LIVE head, never from the next number a spec happens to see (they collided once).** |
