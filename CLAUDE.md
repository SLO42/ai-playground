...
| **Failure log** | `docs/fails.md` on **`v2-main`** — the AUTHORITATIVE copy. Scan it before starting; append when a gate fails first-try. **⚠️ An `F-NNN` id is AMBIGUOUS across copies (measured 2026-08-06).** The `v2` copy is a FORK with its own numbering, holding ~28 entries absent upstream and missing ~12 that exist upstream. **F-016, F-020, F-045, F-046 name DIFFERENT failures in each copy**, and every F-id cited in this file means the **authoritative** copy — which resolves WRONG in a `v2` worktree, where 25 of the 26 worktrees on disk live. **ALLOCATE BY READING BOTH FILES, NEVER BY QUOTING A NUMBER — including a number on this line.** Take the highest id present in either copy and go one past it; never allocate from a branch-local head, which is exactly what caused the collisions, and never honour a paper reservation. *(This row previously cached a head and a "next free" id; within hours it was stale AND self-contradicting — the same drift-by-one that hit the fork marker a day after it was written. A cached count is the defect, not the cure: no number is quoted here now, on purpose.)* Measurement + reconciliation options: `docs/FAILS-DIVERGENCE.md` (operator-gated). |
...
- **A record id from a request is TABLE-SCOPED at the repo, never shape-only (F-062 — SEVEN instances, promoted per §6).** A bare `UPDATE $rid` / `DELETE $rid` / `SELECT … FROM ONLY $rid` reached by a route handler must pin the expected TABLE (`assertRecordIdOfTable`, `db/validate.ts`) inside the repo function, not at the callsite — `retagTask`, `moveTask`, `completeSprint`, `removeTarget`, `launchSession`, `pmWithdraw` and `pmRevise` each had the same hole and each was found separately; the last two were found by the red-team **on the page the first fix had just hardened**. Worst observed: `removeTarget(db,'project:<slug>')` **deleted the project row** and answered `200 {ok:true}`; `pmRevise` **created a task in another project** and reported `ok:true`. **A state machine is not a table guard**: `canTransition` only asks whether the row's current status string is a key of `ALLOWED_TRANSITIONS`, so every table whose status enum overlaps is writable through it. The route adds the PROJECT check (only the route knows the URL's project); the repo adds the TABLE check (only the repo knows the write). Shape-only validation passes a well-formed foreign id straight through — and the id being bound as a `$param` (D-016) does not help, because the exposure is the bound record link *pointing at a foreign row*.
