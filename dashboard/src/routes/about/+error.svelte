	...
	$effect(() => {
		if (page.error) {
			Sentry.captureException(page.error, {
				tags: { route: '/about', status: page.status, component: 'about-page' }
			});
		}
	});
</script>
...
<div class="flex items-center justify-center min-h-[60vh]">
	<div class="text-center">
		...
		<h1 class="text-xl font-bold text-text-primary mb-2">About Error</h1>
		<p class="text-sm text-text-secondary mb-6">{page.error?.message ?? 'Failed to load page.'}</p>
