	...
	$effect(() => {
		if (page.error) {
			Sentry.captureException(page.error, {
				tags: { route: '/channels', status: page.status, component: 'channels-page' }
			});
		}
	});
</script>
...
<div class="flex items-center justify-center min-h-[60vh]">
	<div class="text-center">
		...
		<h1 class="text-xl font-bold text-text-primary mb-2">Channels Error</h1>
		<p class="text-sm text-text-secondary mb-6">{page.error?.message ?? 'Failed to load channels.'}</p>
