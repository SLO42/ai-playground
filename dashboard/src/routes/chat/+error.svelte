	...
	$effect(() => {
		if (page.error) {
			Sentry.captureException(page.error, {
				tags: { route: '/chat', status: page.status, component: 'chat-page' }
			});
		}
	});
</script>
...
<div class="flex items-center justify-center min-h-[60vh]">
	<div class="text-center">
		...
		<h1 class="text-xl font-bold text-text-primary mb-2">Chat Error</h1>
		<p class="text-sm text-text-secondary mb-6">{page.error?.message ?? 'Failed to load chat.'}</p>
