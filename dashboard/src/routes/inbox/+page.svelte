<script lang="ts">
	...
	let loaded = $derived(data?.conversations != null);
					...
					<div class="flex-1 p-4 space-y-3 overflow-y-auto max-h-[400px]">
						{#each selected.messages as msg}
							<div class="flex {msg.isAgent ? 'justify-start' : 'justify-end'}">
								<div class="max-w-[90%] sm:max-w-[80%] rounded-lg px-3 py-2 {msg.isAgent ? 'bg-bg-primary border border-border' : 'bg-accent-blue/20 border border-accent-blue/30'}">
									<div class="flex items-center gap-2 mb-1">
					...
					<div class="p-4 border-t border-border">
						<div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
							...
							<div class="flex gap-2">
								...
								<button class="flex-1 sm:flex-none px-3 py-2 text-xs font-medium text-text-secondary border border-border rounded-lg hover:bg-bg-tertiary transition-colors">Skip</button>
							</div>
						</div>
					</div>
				</div>
