	...
	$effect(() => {
		if (page.error) {
			Sentry.captureException(page.error, {
				tags: { route: '/projects', status: page.status, component: 'projects-page' }
			});
		}
	});
</script>
...
<div class="flex items-center justify-center min-h-[60vh]">
	<div class="text-center">
		...
		<h1 class="text-xl font-bold text-text-primary mb-2">Projects Error</h1>
		<p class="text-sm text-text-secondary mb-6">{page.error?.message ?? 'Failed to load projects.'}</p>
