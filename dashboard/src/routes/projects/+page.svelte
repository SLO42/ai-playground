<script lang="ts">
	...
	let loaded = $derived(data?.projects != null);
...
<div class="space-y-6">
	...
	<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
		<div>
			...
			<p class="text-sm text-text-secondary mt-1">
				{data.projects?.length ?? 0} projects registered
			</p>
		</div>
			...
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
				{#each filtered as project (project.id)}
					...
					>
						...
						<!-- Stats Row -->
						<div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-text-secondary pt-3 border-t border-border">
							<span>{project.agents} agents</span>
							<span>{project.sessions} sessions</span>
							<span>{project.memoryNodes} memory</span>
							<span class="sm:ml-auto font-mono">{project.lastOpened}</span>
						</div>
					</a>
				{/each}
			</div>
	...
	{:else}
		<!-- Loading skeletons -->
		<Skeleton variant="text" lines={1} height="2.5rem" />
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
			{#each Array(4) as _}
