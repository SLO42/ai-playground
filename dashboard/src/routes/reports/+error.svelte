	...
	$effect(() => {
		if (page.error) {
			Sentry.captureException(page.error, {
				tags: { route: '/reports', status: page.status, component: 'reports-page' }
			});
		}
	});
</script>
...
<div class="flex items-center justify-center min-h-[60vh]">
	<div class="text-center">
		...
		<h1 class="text-xl font-bold text-text-primary mb-2">Reports Error</h1>
		<p class="text-sm text-text-secondary mb-6">{page.error?.message ?? 'Failed to load reports.'}</p>
