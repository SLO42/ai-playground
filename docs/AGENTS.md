...
> ⚠️ **Intersects D-032 (OPEN — single store vs polyglot).** "Serve the body from the DB at activation" assumes bodies live in SurrealDB. The competing agentskills.io convention (§8) keeps the full body in the on-disk `SKILL.md`. These pull opposite directions: **bodies-in-DB** (lazy-load, kongcode) vs **bodies-in-files** (git-diffable, hermes). Author the stub + loader pattern *so it works either way* — don't hard-code a DB fetch into the authored skill — and let D-032 settle where bodies actually live.
...
> ⚠️ **This is one side of D-032 (OPEN — single SurrealDB store vs polyglot).** Adopting agentskills.io files means **skills are git-diffable files on disk** (the hermes/polyglot side), which is *already* consistent with D-010 (Claude Code config is filesystem-authoritative, DB mirrors). But it sits in tension with §5's "serve bodies from the DB," and **adopting it for skills partially pre-commits the D-032 question toward files-for-skills.** Surface this — don't let the choice of file standard silently decide the store architecture. D-032 should resolve it explicitly; this doc flags the entanglement.
...
- **D-032 (OPEN — single SurrealDB store vs polyglot) ↔ skills-as-files vs bodies-in-DB.** Cross-referenced in **§5** (kongcode: serve bodies from the DB) and **§8** (hermes/agentskills.io: SKILL.md files on disk). Adopting the agentskills.io file standard **partially pre-commits** D-032 toward files-for-skills, and it already aligns with D-010 (filesystem-authoritative config). **Flagged, not decided** — D-032 must settle: do skills live as git-diffable files (mirror in DB), or as DB rows (files generated)? The stub+loader pattern (§5) is authored to survive either outcome.
...
- **Telemetry sidecar ↔ utilization loop.** The `.usage.json` sidecar (§8) is where per-skill outcome signal will land; wiring it into ranking + keep/prune is the deferred self-improvement loop (D-022, BRIEF §2.1). Author the sidecar now even though nothing reads it yet — it's the seat the loop plugs into later.
