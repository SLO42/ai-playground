...
**(a) Session orchestration** (`server/claude-code`) — launches Claude Code sessions (interactive or headless), maps them to `session` records carrying `cc_session_id`, streams **transcript events** (assistant/tool/usage) to the event bus, and exposes control actions: **interject** (the named **`channel.pushToSession(ccSessionId, {origin, kind, body})`** primitive — the `claude/channel` push proven by claude-peers, D-035 — with `origin` stamped server-side at authenticated ingress; `origin = operator` only with the D-025 token, else fenced as `agent` data), **stop** (cancel), **resume** (by `cc_session_id`). Fleet view = query of running sessions (D-011). **Runtime mechanism (S1-proven, D-002):** the **SDK is primary** for headless/programmatic runs (clean, deterministic); the **CLI** (for interject/resume parity) is spawned with an **isolated config** (dedicated `CLAUDE_CONFIG_DIR`/`--settings`, harness gates/hooks only) so driven agents never inherit the operator's personal plugins/hooks. Concurrency is safe (no v1 hang).
...
**Communication substrates — two transports, one rule (D-035).** There are exactly two comms transports, and **every feature must pick one** — no module invents its own socket/poller/daemon:
- **(A) the `events` bus** — the SOLE fan-out for everything **observe / notify / broadcast / state-out**: transcripts, task / workflow-step / `work_item` changes, metrics, notifications, incidents, the fleet "what's each session doing" summary. (This is the bus above.)
- **(B) `claude/channel` push** — the SOLE transport for delivering a message **INTO a running Claude Code session**: the `channel.pushToSession(ccSessionId, {origin, kind, body})` primitive (interject today, agent→agent push later). It carries a **mandatory, server-stamped, immutable `origin`**; it is **out-of-band** to the dashboard SSE.
...
**Abuse case — channel-push spoofing (D-035).** The `claude/channel` push (`channel.pushToSession`) is the **one** thing allowed to issue *instructions* into a running agent (not just data). If it accepted any loopback sender, (a) any other local process (CSRF / DNS-rebind — **loopback ≠ proof of operator origin**) could inject operator-grade instructions ("run this bash", "ignore your gates") into a live agent, and (b) a prompt-injected agent that gains the send capability could escalate its own data-only message into a *steering* message against a peer — going **around** the D-026 content fence. **Mitigation:** steering authority is **unforgeable without the D-025 per-boot token**; the **agent runtime is NEVER handed the token** (token == the operator's steering capability — if the agent holds it, agent == operator); `origin` is stamped server-side at authenticated ingress and **unknown/unauthenticated origin fails closed → `agent`** (fenced as data). Agents get a send capability hardwired to `origin = agent` only.
