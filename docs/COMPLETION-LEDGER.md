...
Add 3 permanent checklist lines to the v2-wave reviewer `commonExtra` so EVERY wave's independent D-038 reviewer checks: (1) analytics first-class on every new decision path, (2) honest failure states + no silent catch without a happy-path test, (3) human-readable + accessible + design-tokens. Pass/fail per feature, adjective-free.
