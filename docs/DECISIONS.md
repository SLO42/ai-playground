...
## D-042 🔒 Versioned capability artifacts with server-stamped audit trails (2026-07-23)

**Context:** the operator required that **tools, agents, and skills version, with audit trails showing WHERE an update came from, WHY, and WHEN** — the substrate that makes autonomous self-improvement (D-022) safe and reversible. Prior art already exists in-house: `role_version` (`schema.ts:1077`) versions roles correctly today — integer `version`, `prompt_sha` content hash, `source` enum, supersession via lifecycle + pointer, an immutable F-048-safe `dedup_key` (`role|version`, never keyed on mutable status), and a separate append-only `role_event` audit table. But `cc_agent`/`cc_skill` are hash-less snapshots (sync overwrites them; git is the only history), and there is no in-product trail the brain, the dashboard, or the effectiveness loop can query without shelling to git — nor any join from a running session back to the exact artifact versions it used.

**Decision:** every mutable capability artifact (tool/capability, agent definition, skill, workflow, rule, rejection, role) is an **append-only versioned entity**. Each version records, in-product and queryably: `version` (monotonic), `content_hash` (EOL-normalized — a CRLF-only diff mints no version, F-054), **SOURCE** (server-stamped from a closed vocabulary — `operator` / `sync_from_disk` / `hr_recruiter` / `self_improvement` / `brain_observer` / `rollback` — **unforgeable, D-035a-style**: the server asserts the source from the invoking code path, never from the artifact content or any LLM payload), **REASON** (a link to the causing git-commit / learning_candidate / validation_run / interview_run / operator-consent; NULL when honestly unknown, never fabricated), **WHEN** (ISO), and what it **SUPERSEDES**. Autonomous (non-operator, non-sync) version bumps **require a linked, passed validation gate** — fail-closed here is correct (D-024 scope). All writes route through **one chokepoint per version table** (F-055; `writeArtifactVersion` for the unified table, `createRoleVersion` for roles). **Rollback appends a new version pointing at its target; history is never deleted.** Git remains disk-truth for `.claude/*` artifacts (D-010); the in-product trail mirrors it at sync time and reconciles by `content_hash`. `session.artifact_versions` pins the versions each session ran with → effectiveness attribution + regression tracing.

**Consequences:** spec → `docs/VERSIONING-AUDIT-SPEC.md`; queued in BUILD-QUEUE (`versioning-audit`, gate:operator). **Generalizes `role_version`** (roles stay on it — live, tested, gated; the unified surface reads by read-time UNION, so no second writer, F-055). Reuses append-only knowledge discipline (D-015/D-028). 9 named safety invariants. Phased v1(substrate + sync-born chains + read-only history UI)/v2(session→version attribution join)/v3(HR + self-improvement writers + operator-gated rollback). Surfaces on `/agents`, `/skills`, `/claude-code`, `/brain` (per `NAV-IA-MAP.md`). This trail is the **reversibility guarantee** underpinning the self-improvement loop (D-022) and the brain-observer's adopted changes. LOCKED 2026-07-23 (operator).
...
| D-042 | 🔒 | Versioned capability artifacts + server-stamped audit trails (source/reason/when/supersedes; unforgeable source; autonomous bumps need a passed validation gate; rollback appends never deletes; generalizes role_version) — VERSIONING-AUDIT-SPEC.md |
