...
- **2.10 session control →** **[claude-code]** *(1.6)* — interject / stop / resume; fleet view (D-011; mechanism from S1). **Build interject through the named `channel.pushToSession` seam** (D-035): every message carries a server-stamped, immutable `origin`; `origin = operator` only when the push bears the D-025 token on the loopback control endpoint, else forced to `origin = agent` and fenced (D-026). The agent runtime never holds the token. `verify: a forged/tokenless interject is downgraded to origin=agent and does NOT steer the running session; an operator interject (with token) does`. The v0.2 `peer_message` fleet bus (D-035b) lands as an additive consumer of this same seam — not pulled earlier.
