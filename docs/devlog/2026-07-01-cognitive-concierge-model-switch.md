...
| **self-maintenance loops (roadmap capstone)** | `2c6e638` | generic `MaintenanceLoopEngine` (cadence runner: mode gate → enabled → readiness green-or-override → registered in-code action; per-loop minute dedup + in-flight guard; fail-open F-048; kill-switch re-read every tick) + 2 declared loops: `maint:eval-regression` (retrieval-quality measurement on cadence, m0080 `eval_report`) and `maint:reranker-eval` (counts S2 labels → retrains → PROPOSES enabling, never flips the default — D-004). Honest catch: `runEval` seeds fixtures → actions run in a THROWAWAY spawned surreal so live memory is never poisoned. Ships declared-not-running until operator arms via /loops. Live-verified (engine boots, both loops render); m0080 live 80/80. F-056 logged (`*/` in JSDoc) |
| advisories FIFO origin-filter | `7c303f9` | legacy FIFO pairing admits only concierge-origin rows (worker mailbox rows can never mis-pair as advisory replies); deleted-sender rows excluded (honest default) |
| worker conversation layer completed (pillar 3) | `a6be543` | scout found ~90% pre-built (write-intent workers already granted peer-send; guards/visibility free). Fixed the 3 real gaps: PM-inbox drain now drains BOTH message shapes with honest sender labels — worker `{kind:'pm'}` rows previously had NO drain at all (silently stranded) + everything was mislabeled "concierge advisory"; workflow-runner steps now get capabilities; when-to-talk guidance (blocked→PM, relevant finding→peer, never command). "Hires actually converse" is live |
