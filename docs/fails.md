...
## F-046: a Svelte 5 `$effect` that READS the same `$state` it WROTE in its own body self-loops → `effect_update_depth_exceeded` (build/test/lint/svelte-check all GREEN — only live-verify catches it)
- **Date**: 2026-06-22
- **What**: building the ProjectActivity panel I rendered the project's live transcript on the Overview tab (a second consumer of the existing `liveTurns` `$state`). On a runtime selection change the page threw `effect_update_depth_exceeded — Maximum update depth exceeded… an effect reads and writes the same piece of state` (console, during the client-side `?session=` toggle and on a deep-linked hydrate). Build, `npm test`, `npm run lint` (0), and `svelte-check` (0/0) were ALL green — the loop only manifested in the running browser. The trigger was a latent line in the pre-existing live-transcript `$effect`: it wrote `liveTurns = (data.transcript ?? []).map(...)` and then, a few lines later in the SAME effect body, read `let liveSeq = liveTurns.length` — reading the `$state` it had just written. (A first wrong hypothesis blamed the auto-switch-to-Sessions `$effect`; "fixing" it with a guard flag I then read+wrote in its own body — `if (autoTabApplied) return; autoTabApplied = true;` — was the SAME bug a second time and made the loop fire on plain hydrate. Two strikes, same class.)
- **Why**: in Svelte 5, reading a rune `$state` inside an `$effect` registers it as a dependency of that effect. If the effect's own body also WRITES that state, the write re-dirties the dependency → the effect re-runs → writes again → depth-exceeded. It had been latent because the transcript previously rendered on ONE tab; adding the Overview consumer kept the effect graph "hot" enough on selection-change that the self-dependency materialized into a full loop. The build-time checks can't see it: it is a runtime reactive-graph cycle, not a type/compile/lint error.
- **Fix**: (1) live-transcript effect — build the seed into a LOCAL `const seed = (data.transcript ?? []).map(...)`, assign `liveTurns = seed`, and seed the seq counter from `seed.length` (the local), NEVER reading `liveTurns` back inside the effect. (2) one-shot deep-link tab default — DON'T use an `$effect` for "do this once at init"; initialize the `$state` directly from the loader value wrapped in `untrack(() => …)` so the compiler knows it is a deliberate one-time read (and `svelte-check`'s `state_referenced_locally` warning, which lint promotes to an error, stays clean). Live-verified: 0 console errors on fresh load, on the in-page expand toggle, and on a deep-linked `?session=` hydrate.
- **Prevention**: NEVER read a `$state`/`$derived` inside the same `$effect` that writes it — if the effect needs the value it is about to store, compute it into a `const` LOCAL first and use the local for both the assignment and any follow-on (length/seq/merge) logic. For "run exactly once at init" prefer a direct `$state(untrack(() => initialValueFromData))` over an `$effect` with a self-read+write guard flag (that guard IS the bug). This whole class is INVISIBLE to build/test/lint/svelte-check — the bounded live-verify (open the page, change the reactive input, watch the console for `effect_update_depth_exceeded`) is the only gate that catches it, so a feature that adds a second consumer of an existing reactive store MUST be live-verified, not signed off on a green build alone (F-014 server-discipline already mandates the live pass; this is why).

## F-047: a "rescued one path" fix that leaves the SIBLING paths with the IDENTICAL anti-pattern (+ a stray NUL separator byte in a hashed key)
- **Date**: 2026-06-24
- **What**: SH-5's first fix de-raced `cc_settings` (deterministic-id guarded-UPSERT replacing a non-atomic DELETE-then-CREATE) and the commit body claimed it "closes the latent cc_skill-multiplication race" — but `replaceSkills`/`replaceHooks`/`replaceAgents`/`replaceMcpServers` (`cc-config/sync.ts`) still did the EXACT non-atomic `DELETE cc_skill WHERE scope` + per-row `CREATE`. The digest-gate only short-circuits in STEADY state; during the drift-transition window (a real disk change — exactly when SH-3 promotes a skill) all N concurrent reconciles read the OLD stored digest, all pass the gate, and all race the child replace → cc_skill duplicated 3x-8x for one skill (review's 8-parallel probe). `catalogIds` (a `Set`) was immune, so the test suite stayed GREEN while `readCatalog` (no dedup) rendered the duplicate rows → mirror lies vs disk (F-008/D-010). Separately, while typing the new `childIdOf` hash key, the separator landed as a literal NUL byte instead of a space — git flagged the source file `binary`, and a NUL is a silent key-collision hazard.
- **Why**: (1) a fix scoped to the ONE path that reproduced the symptom, when the SAME anti-pattern lives verbatim in N sibling functions, only moves the bug — the sibling paths are still wrong and a different trigger (here: the promote-time drift window, not steady state) re-exposes it. (2) A `$param`-bound value (`catalogIds`'s `Set`) masking the duplication meant the unit suite couldn't see it; the honesty surface (`readCatalog`, no dedup) was the only place the lie showed, and nothing asserted it. (3) The NUL byte slipped in because a keystroke emitted U+0000 for the separator and no build check flags a NUL in a template literal (svelte-check/lint/tsc all passed; only `git diff` calling the file "binary" exposed it).
- **Fix**: extracted a `replaceChildren(db, table, scope, rows)` helper doing GUARDED-SWEEP (`DELETE <table> WHERE scope = $scope AND id NOTINSIDE $keep`, table name through `assertTableName` D-016) + per-row `UPSERT $rid CONTENT`; gave every child a DETERMINISTIC id `childIdOf(table, prefix, scopeId, key)` keyed by its disk identity (`file_path` for agents/skills, `name` for mcp, full content tuple for hooks). Concurrent writers compute identical ids → converge on one row. Replaced the NUL with a `|` (the separator `scopeIdOf` already uses). Added a regression test (8-parallel forced-drift) asserting cc_skill has zero duplicate ids AND each name appears once in `readCatalog` — proven RED on the old code, GREEN on the fix.
- **Prevention**: when a fix de-races / hardens ONE code path, GREP for the same anti-pattern (here `DELETE <table> WHERE scope` + `CREATE … CONTENT`) across its sibling functions and fix them in the SAME change — a commit body must not claim to close a race it only closed in one of N identical paths. If a `Set`/dedup downstream masks a duplication bug, add a regression assertion on the RAW (un-deduped) surface (row count / `readCatalog`), not only on the deduped one. And: a hashed/composite KEY separator must be a visible literal (`|`/`\x1f`), never a typed space that can land as NUL — if `git diff` ever reports a `.ts` as "binary", search for a NUL byte (`fs.readFileSync(p).indexOf(0)` ≥ 0) before committing.
...
## F-057: a shared dedup INDEX/VALUE has MORE producers than the obvious one — changing it breaks the others
- **Date**: 2026-07-07 (BL-R3 — F-048 task-status recovery completeness)
- **What**: fixing the F-048 task_run concurrent-clobber, I switched `work_item` active-window dedup to a DETERMINISTIC PRIMARY id (workqueue.ts `enqueue`, per F-026 — a secondary UNIQUE index over a computed VALUE does NOT enforce under concurrency on this build) and, to "match the proven-clean file_snapshot pattern", ALSO shipped a migration that dropped `UNIQUE` from `work_item_dedup` + collapsed the `dedup_key` VALUE. The full suite then failed `fast-tier-drain.test.ts (3)`: `enqueueReview` (memory_review) expected a dedup no-op (null) but got a real second row. Root cause (Iron Law — reproduced by the red test): `work_item` has THREE OTHER direct `CREATE work_item` producers besides `workqueue.enqueue()` — `memory/loop.ts enqueueReview`, `workforce/activation.ts` (reinterview), `workforce/gauntlet.ts` (queued-interview) — each CREATEs with a RANDOM id and relies on the `work_item_dedup` UNIQUE index for its dedup. Dropping UNIQUE silently removed THEIR dedup.
- **Why**: I reasoned "enqueue() is the sole producer, so the index is redundant" from the ONE caller I was editing, without grepping every `CREATE work_item`. A dedup mechanism (a UNIQUE index / a computed VALUE) is shared infrastructure; its consumers are every direct writer of the table, not just the function in front of me.
- **Fix**: reverted the migration entirely (kept the UNIQUE index + old VALUE untouched). The deterministic-id guard in `enqueue()` is fully sufficient for task_run and is UNAFFECTED by the index (the id is the guarantee; the same unit is always the same single row, so the index never false-fires). The 4-concurrent proof test passes WITH the index present. Routing the other three producers through the same deterministic id is written down as DEFERRED (out of BL-R3 scope), not silently done.
- **Prevention**: before changing or removing a shared dedup INDEX/VALUE (or any table-level constraint), grep EVERY direct writer — `grep -rn "CREATE <table>"` (and UPSERT/INSERT) — and confirm each one's dedup still holds after the change. A table's dedup consumers are all its producers, not just the one you're editing. Prefer scoping a fix to the code path in your task (here: the deterministic id in `enqueue()`) over a schema-wide change that silently alters every other producer's behavior.
