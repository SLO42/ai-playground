...
<section class="page">
        ...
        <ul class="fleet-rows" aria-label="sessions across all projects">
          {#each [...running, ...recent] as s (s.id)}
            ...
            <li class="fleet-row" class:running={isRunning} class:selected={isSelected}>
              ...
              <!-- OBSERVABILITY — the honest failure reason on a failed session. session.note is
                   D-026-screened at the launch write (a secret was redacted before it landed), so
  ...
  {#if connected && selectedSession}
    <section class="card transcript-panel" aria-label="session transcript">
      ...
      <!-- OBSERVABILITY — the honest failure reason for the OPEN session (D-026-screened at the
